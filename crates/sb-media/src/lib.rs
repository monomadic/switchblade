...
/// Read-ahead depth for the live decode queue: enough to ride out a slow
/// frame, small enough that an unwatched player stalls (and stops burning
/// CPU) almost immediately. Shared by every live lane's `SeekablePlayer`.
