...
**Shipped as `SeekablePlayer`** (`crates/sb-media/src/seekable.rs`): the same paced-queue contract the old CLI-pipe player presented (`spawn`/`take_frame`/`buffered`) plus `seek(f64, exact)`; demuxer + decoder owned by the reader thread, `seek()` = flush + `avformat_seek_file` backward (+ decode-forward when exact), position is a real pts property (content-relative — start_time/edit-list offset applied on seek, subtracted on position, so it matches the CLI `-ss` thumbnail seek). The decode/scale chain reuses `hw_scale_vf` verbatim as a libavfilter graph (parity by construction), the sw fallback rotates explicitly (libavfilter doesn't autorotate), pacing stamps by pts-delta with late re-anchor. All live lanes — selected, warm pool, hover — run on it; `[`/`]` is a plain `seek()` and the old `warm_skip`/checkpoint machinery is deleted. The ffmpeg-CLI `LivePlayer` was **removed 2026-07-22** once the in-process path had soaked (git history preserves it for reference). Tests: the `SeekablePlayer` pacing/stall/drop trio + `seek_jumps_in_place_without_respawn` + `start_time_offset_is_content_relative` + `skip_seeks_the_selected_stream_in_place`.
