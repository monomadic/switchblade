...
4. ✅ **Filmstrip hover-play.** The hover lane for chips partially exists (hovered chips scale and get a tile-size lane) — audit why it doesn't *feel* like it plays: likely the grid's hover settle delay and warm-pool priorities apply where quickview should be eager. Chips the strip already pre-warms (±1) should show video instantly on hover via promotion, same rule as selection moves. *(Audit result: the settle delay was the culprit — quickview chips now start their lane immediately; the remaining cold-spawn latency is the CLI `LivePlayer`'s, which shrinks when the hover lane ports to `SeekablePlayer` post-soak.)*
